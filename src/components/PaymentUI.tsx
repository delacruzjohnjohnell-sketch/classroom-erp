...
export function RecordPaymentForm({ balance, onSubmit }: { balance: number; onSubmit: (amount: number, date: string, method: string) => void }) {
  const [amount, setAmount] = useState(String(balance.toFixed(2)));
  const [date, setDate] = useState(todayStr());
  const [method, setMethod] = useState("cash");

  return (
    <form onSubmit={(e) => { e.preventDefault(); const n = parseFloat(amount); if (!n || n <= 0) return; onSubmit(n, date, method); }}>
      ...
      <button type="submit" className="primary-btn mt-4">Record payment</button>
    </form>
  );
}
